...
import { objectToDynamo, realAttributesToDynamo } from '../lib/utils/realToDynamoTypes'
...
test('Query filter is being generated', async () => {
  const generateKeyConditionExp = (configs: QueryItemsConfig) => {
    ...
    const conds = {
      ':pk': {
        [realAttributesToDynamo(pkValue)]: pkValue instanceof DynamoBinary ? pkValue.value : pkValue
      }
    }
    ...
    if (skValue) {
      return {
        ...
        keyFilters: _.merge(conds, {
          ':sk': {
            [realAttributesToDynamo(skValue)]: skValue instanceof DynamoBinary ? skValue.value : skValue
          }
        })
      }
    } else {
